...
**一期（已完成）**：TUI 连续对话、流式输出、会话 jsonl 存储与 `-c` 恢复、plan/build 模式切换（Tab / `/plan` `/build`）、只读工具（read_file/grep/glob/web_fetch/run_command 只读白名单）、**build 写能力**（write_file/edit_file/delete 直接执行；run_command 全量 + 危险命令 TUI 确认）、Tab 切换模式、ASCII 边框（CJK locale 兼容）。
